...
up:
	cd ./env && export COMPOSE_PROJECT_NAME=$(TAG); export APPSETTING_UID=$(UID); export APPSETTING_GID=$(GID); export APPSETTING_USER=$(USERNAME); export APPSETTING_DOCKER_GROUP_ID=$(DOCKER_GROUP_ID); docker compose up -d --force-recreate --build && echo && docker ps --filter name=^/$(TAG)-app
down:
