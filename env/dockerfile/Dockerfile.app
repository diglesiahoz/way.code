...
ARG APPSETTING_SOURCE_IMAGE
FROM $APPSETTING_SOURCE_IMAGE
...
# apt network hardening (VPN-friendly):
...
# switch mirrors to HTTPS (HTTP 80 may fail behind VPN)
...
# bootstrap TLS trust on HTTPS (temporary no-verify)
RUN printf '%s\n' \
    'Acquire::https::Verify-Peer "false";' \
    'Acquire::https::Verify-Host "false";' \
    > /etc/apt/apt.conf.d/05bootstrap-insecure-https \
    && apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates \
    && update-ca-certificates \
    && rm -f /etc/apt/apt.conf.d/05bootstrap-insecure-https \
